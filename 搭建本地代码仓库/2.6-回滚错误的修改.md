...
理解这一点很重要。`git reset` 回滚了「单独一个提交」，它移除了后面的提交，然后回到项目之前的状态。
...
如果说 `git revert` 是一个安全的撤销方式，你可以将 `git reset` 看做一个 *危险* 的方式。当你用 `git reset` 来重设更改时（提交不再被任何引用或引用日志所引用——提交没用了），我们无法获得原来的样子——这个撤销是永久的。使用这个工具的时候务必要小心，因为这是少数几个可能会造成工作丢失的命令之一。

和 `git checkout` 一样，`git reset` 有很多种用法。它可以被用来移除提交快照，尽管它通常被用来撤销缓存区和工作目录的修改。不管是哪种情况，它应该只被用于 *本地* 修改——你永远不应该重设和其他开发者共享的快照。

### 用法

`git reset`总会让当前的分支头节点指向指定回退的提交节点位置，但它有三个参数，分别是 `--soft`， `--mixed`， `--hard`，我们来介绍下这三个参数有什么不同。当我们指定一个过去的提交并用 `git reset`来回退时，如果是 `--soft`参数，回退后，比现在更新的一些修改会放到缓存区中；如果是 `--mixed`参数，回退后，比现在更新的一些修改会放到工作区中，这是不加这三个参数时的默认值；如果是 `--hard`参数，回退后，比现在更新的一些修改会被直接丢弃（再找不回来了，所以要谨慎操作）。

```
git reset <file>
```

从缓存区移除特定文件，并将更改放到工作区中，不覆盖任何其他更改。（隐式--mixed参数）

```
git reset
```

重设缓冲区，匹配最近的一次提交（HEAD），并将更改放到工作区中。相当于是取消所有文件的缓存，而不会覆盖任何其他修改，给你了一个重设缓存快照的机会。（隐式--mixed参数）

```
git reset --hard
```

重设缓冲区和工作目录，匹配最近的一次提交（HEAD）。它会清除所有未提交的更改，包括缓存区和工作目录的更改，所以在使用前确定你想扔掉你所有本地的开发。

```
git reset <commit>
```

将当前分支的末端移到 `<commit>`，将比这个提交更新的更改放到工作区，这允许你用更干净、原子性的快照重新提交项目历史。（隐式--mixed参数）

```
git reset --hard <commit>
```

将当前分支的末端移到 `<commit>`，它不仅清除了未提交的更改，同时还清除了 `<commit>` 之后的所有提交。

```
git reset --soft <commit>
```

将当前分支的末端移到 `<commit>`，将比这个提交更新的更改放到缓冲区。

### 讨论

上面所有的调用都是用来移除仓库中的修改。没有 `--hard` 标记时 `git reset` 通过取消缓存或取消一系列的提交，然后重新构建提交来清理仓库。而加上 `--hard` 标记对于作了大死之后想要重头再来尤其方便。

撤销(revert)被设计为撤销公共分支的提交的安全方式，`git reset`被设计为重设本地个人分支的更改。因为两个命令的目的不同，它们的实现也不一样：reset完全地移除了一堆更改，而revert保留了原来的更改，用一个新的提交来实现撤销。
...
`git reset --hard HEAD~2` 命令将当前分支向前倒退两个提交，相当于在项目历史中移除刚创建的这两个提交。记住，这种重设只能用在本地私有分支的提交中。绝不要在将提交推送到共享仓库之后执行上面的操作。
