...
比如说 `git clone https://github.com/P2Tree/git-recipes` 可以将 git 教程 clone 到你指定的目录。
...
> - 这里 origin 是 &lt;server&gt; 的别名，取什么名字都可以，你也可以在 push 时将origin替换为\<server>。但为了以后 push 方便，我们第一次一般都会先 remote add，之后用origin来指代远程服务器地址。
> - 如果你还没有 Git 仓库，可以在 Github 等代码托管平台上创建一个空的仓库（不要选择自动生成 README.md，若远程仓库生成了任何文件，则这次push会遇到冲突的问题，我们后续章节会讲解merge的操作），然后将代码 push 到远端仓库。

至此，你应该可以顺利地提交你的项目了。在下一节中，我们将涉及更多的命令，来完成更有用的操作。比如从远端的仓库pull更新并且merge到你的本地，如何通过分支多人协作，如何处理不同分支的冲突等等。