...
这份教程讨论了重写和修改Git历史的一些方法。Git用户可以使用不同的多种方式来修改记录，我们将会讨论这些不同方法的优缺点并给出具体的使用示例。这份教程讨论了重写提交快照的一些常见原因，并告诉你如何避免不好的影响。

Git 的主要职责是保证你不会丢失提交的修改。但是，它同样被设计成让你完全掌控开发工作流。这包括了让你自定义你的项目历史，而这也创造了丢失提交的可能性。Git 提供了可以重写项目历史的命令，但也警告你这些命令可能会让你丢失内容。

Git 有多种不同的机制来保存历史和变更，这些机制包括：`commit --amend`，`git rebase`以及`git reflog`。这些命令可以给予你高度的自定义工作流的选择。在这份教程的结尾，你将会对这些重构你Git提交历史的命令有一个清晰的认识，并且能尽可能避免因此修改会带来的问题。
...
加上`-m`参数可以在不打开编辑器的同时修改上一次提交的commit message。
...
在[`git reset`](https://github.com/geeeeeeeeek/git-recipes/wiki/2.6-%E5%9B%9E%E6%BB%9A%E9%94%99%E8%AF%AF%E7%9A%84%E4%BF%AE%E6%94%B9#git-reset)这节中，我们说过永远不要重设和其他开发者共享的提交。对于`git commit --amend`也是一样：永远不要修复一个已经推送到公共仓库中的提交。

修复过的提交事实上是全新的提交，之前的提交会被移除出项目历史（类似于先`git reset`然后再`git commit`）。这和重设公共快照的后果是一样的。如果你修复了其他开发者在之后继续开发的一个提交，看上去他们的工作基础从项目历史中消失了一样。对于在这上面的开发者来说这是很困惑的，而且很难恢复。
...
你可以使用`git rebase`来将一系列过去的旧提交合并成一个新的提交。标准的使用中，`git rebase`允许你从字面上重写历史记录——自动将当前的工作分支的提交应用于传递的分支头。因为你的新提交会替代那些旧提交，所以切记不要用在公共代码中，这可能会导致一些很麻烦的合并问题。

rebase的操作机制是将分支移到一个新的基提交的过程。过程一般如下所示：
...
rebase 的主要目的是为了保持一个线性的项目历史。要将你的 feature 分支整合进 `master` 分支，你有两个选择：直接 merge，或者先 rebase 后 merge。前者会产生一个三路合并（3-way merge）和一个合并提交，而后者产生的是一个快速向前的合并以及完美的线性历史。
...
交互式 rebase 给你了控制项目历史的完全掌控。它给了开发人员很大的自由，因为他们可以先专注于写代码而不必太在意提交历史，完成代码之后，再使用交互式rebase来整理提交历史。
...
![Git Tutorial: git rebase -i example](https://www.atlassian.com/dam/jcr:2d03f5b6-eaa6-4e78-9dd7-3686ba2a7665/05.svg)
...
交互式 rebase 强大的能力可以从整合后的 master 分支看出——额外的 `62eed47` 提交找不到了。对其他人来说，你就像是一个天才，用完美数量的提交完成了 `new-feature`。这就是交互式提交如何保持项目历史干净的方法。

另外，再介绍一些交互式rebase的操作选项。在交互式rebase时，将pick修改为edit，这将会暂停rebase操作，并且退出交互式界面，然后会回退到你选择edit的这个提交，并允许你去使用`git commit --amend`做一些额外的修改，当修改完成后，通过执行`git rebase --continue`命令来继续rebase的操作。

另外，如果需要修改多个提交记录的message，可以将pick修改为reword。如果选择修改为fixup，那么这个命令的作用和squash类似，但是它不会中断rebase交互并启动修改commit message的界面，它将会直接把这些commit合并并使用最新一个提交的message作为新合并提交的message。

## git reflog

Git 用引用日志这种机制来记录分支顶端的更新或其他提交应用的信息。它允许你回到那些不被任何分支或标签引用的更改。在重写历史后，引用日志包含了分支旧状态的信息，有需要的话你可以回到这个状态。
...
上面的引用日志显示了 master 和 2.2 的 branch 之间的相互切换。还有对一个对提交的强制重设（这个动作直接将两个提交删掉了）。最近的活动用 `HEAD@{0}` 标记在上方显示。

如果事实上你是不小心做的reset（并且后悔了），引用日志包含了你意外地丢掉两个提交之前 master 指向的提交 0254ea7。
...
务必记住，引用日志提供的安全网只对提交到本地仓库的更改有效，而且只有移动操作会被记录。另外，记录的reflog条目是有过期日期的，默认是90天，90天之后的记录将会删除掉。
